"use strict";const i=require("electron"),p=require("path"),d=require("fs"),Y=require("crypto"),m=require("child_process"),Z=require("http"),k=require("https"),tt=require("os");function W(t){const e=Object.create(null,{[Symbol.toStringTag]:{value:"Module"}});if(t){for(const n in t)if(n!=="default"){const r=Object.getOwnPropertyDescriptor(t,n);Object.defineProperty(e,n,r.get?r:{enumerable:!0,get:()=>t[n]})}}return e.default=t,Object.freeze(e)}const et=W(Z),nt=W(k),S=new Map;function _(t){return t.replace(/^(['"])(.*)\1$/,"$2").trim()}function rt(t){return t.startsWith("-")?t:`"${t}"`}function ot(t,e,n,r,o){var j,C;if(q(t))return;const s=_(e.source),a=_(e.destination),u=(e.filters??[]).filter(c=>c.value.trim()!=="").map(c=>`--${c.type}=${c.value.trim()}`),l=[e.type,s,a,...u,"--stats=1s","--use-json-log","--verbose"],g=m.spawn("rclone",l,{stdio:["ignore","pipe","pipe"]});S.set(t,g);const x=`rclone ${l.map(rt).join(" ")}`;n.webContents.send("sync:started",{taskId:t,command:x});let f=null;if(o){const c=p.dirname(o);d.existsSync(c)||d.mkdirSync(c,{recursive:!0}),f=d.createWriteStream(o,{flags:"w"}),f.write(JSON.stringify({level:"info",msg:x,time:new Date().toISOString()})+`
`)}let G="",X="";function R(c,w){c.value+=w.toString();const $=c.value.split(`
`);c.value=$.pop()??"";for(const K of $){const E=K.trim();if(E){f==null||f.write(E+`
`);try{const I=JSON.parse(E);n.webContents.send("sync:progress",{taskId:t,stats:I.stats??null,log:I})}catch{n.webContents.send("sync:progress",{taskId:t,stats:null,log:{level:"warning",msg:E,time:new Date().toISOString()}})}}}}const Q={value:G},z={value:X};(j=g.stdout)==null||j.on("data",c=>R(Q,c)),(C=g.stderr)==null||C.on("data",c=>R(z,c)),g.on("close",c=>{var w,$;f==null||f.end(),S.delete(t),c===0?(n.webContents.send("sync:complete",{taskId:t}),(w=r==null?void 0:r.onComplete)==null||w.call(r)):(n.webContents.send("sync:error",{taskId:t,message:`rclone exited with code ${c}`}),($=r==null?void 0:r.onError)==null||$.call(r))}),g.on("error",c=>{var w;f==null||f.end(),S.delete(t),n.webContents.send("sync:error",{taskId:t,message:c.message}),(w=r==null?void 0:r.onError)==null||w.call(r)})}function st(t){const e=S.get(t);e&&(e.kill("SIGTERM"),S.delete(t))}function it(){for(const[,t]of S)t.kill("SIGTERM");S.clear()}function q(t){return S.has(t)}function L(t){const e=r=>r.replace(/^(['"])(.*)\1$/,"$2").trim(),n=(t.filters??[]).filter(r=>r.value.trim()).map(r=>`--${r.type}=${r.value.trim()}`);return[t.type,e(t.source),e(t.destination),...n]}function F(){const t=process.platform==="win32"?"where":"which",e=m.spawnSync(t,["rclone"],{encoding:"utf-8"});return e.status===0&&e.stdout.trim()?e.stdout.trim().split(`
...
`).trimEnd();N(n?n+`
`:"")}function at(t,e){H(t.id);const n=F(),o=[...L(t),"--use-json-log","--verbose"].map(l=>l.startsWith("--")?l:`"${l.replace(/"/g,'\\"')}"`).join(" "),s=e.substring(0,e.lastIndexOf("/")),a=`${t.schedule} mkdir -p "${s}" && "${n}" ${o} > "${e}" 2>&1 ${O}${t.id}`,u=B().trimEnd();N((u?u+`
`:"")+a+`
